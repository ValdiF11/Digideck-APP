...
function MainCard({ card, fetch, fetchtotal }) {
  async function handleDelete() {
    try {
      const response = await axios({
        method: "DELETE",
        url: import.meta.env.VITE_API_BASE_URL + `/mainDeck/${card.id}`,
        headers: {
  ...
  async function handleIncrement() {
    try {
      const response = await axios({
        method: "PUT",
        url: import.meta.env.VITE_API_BASE_URL + `/mainDeck/increment/${card.id}`,
        headers: {
          Authorization: `Bearer ${localStorage.acces_token}`,
        },
      });
  ...
  async function handleDecrement() {
    try {
      const response = await axios({
        method: "PUT",
        url: import.meta.env.VITE_API_BASE_URL + `/mainDeck/decrement/${card.id}`,
        headers: {
          Authorization: `Bearer ${localStorage.acces_token}`,
        },
      });
